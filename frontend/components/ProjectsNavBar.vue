  ...
  <nav class="w-full h-1/10 flex flex-row justify-between items-center bg-d-bg-primary text-white">
    <div class="w-1/2 flex flex-row py-2 items-center border-1">
      ...
      <div class="flex flex-col pl-6">
        ...
        <a v-if="this.$store.state.otherUserProject === true" @click="viewProfile"><h1 class="h-1/12 w-full flex pl-3 items-center ml-0 text-sm bg-transparent hover:text-gray-400 text-white cursor-pointer">{{this.$store.state.otherUsername}}</h1></a>
      </div>
    </div>
      <div class="h-2/3 w-1/2 md:w-2/5 xl:w-1/4 flex justify-around items-center" v-if="this.$store.state.otherUserProject === false">
        ...
        <button class="bg-gray-500 hover:bg-gray-400 text-white py-2 px-4 rounded w-auto text-sm md:text-md" @click="save"><font-awesome-icon icon="fa-solid fa-floppy-disk" /> Save</button>
      ...
      <div class="h-2/3 w-1/4 md:w-2/5 xl:w-1/4 flex justify-around items-center" v-else-if="this.$store.state.otherUserProject === true">   
        ...
        <img class="basis-5 rounded-full h-10 justify-self-center self-center m-1 cursor-pointer" :src="userProfile.data.profile_pic" @click="viewProfile">
      </div>
</nav>
</template>
